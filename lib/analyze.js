    ...
    async function analyzeFiles(opts) {
        ...
        try {
            ...
            await Promise.all([
                ...
                writeJsonAsync("statsDict.json", statsObj.totalStatsDict),
                writeJsonAsync("tree.json", statsObj.totalTree),
                writeJsonAsync("distinctIncludes.json", statsObj.flatTree),
                writeTextFileAsync("warnings.txt", statsObj.allWarnings),
                writeJsonAsync("analysis.json",statsObj.analysisObj.data),
                writeTextFileAsync("analysis.csv", csv)
            ]);
