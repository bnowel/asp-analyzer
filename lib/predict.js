...
total_loc:1042
*/
        // Sort by num_refs DESC
        predictObj.analysis.sort(function(a,b){return b.num_refs - a.num_refs;});
        
        var fileToMove = predictObj.analysis[0];
        var fileToMoveTo = predictObj.analysis[1];
        var fname = fileToMove.file;
        console.log("Moving include " + fileToMove.file + " to " + fileToMoveTo.file);



        reduceIncludes(predictObj, fname, fileToMoveTo.file);
        ...
        compareModule.compareStats({
            ...
            missingIncludes: nodePath.join(newPath, "missingIncludes.json"),
            summary: true,
            warnings: false
        });
...
function reduceIncludes(predictObj, fname, fnameTo) {
